function via(openId,partTimeJobId){
	if(confirm("确定是否通过")){
	console.log("用户ID:"+openId+"通过兼职id:"+partTimeJobId+"（接上数据库后删除）");  
	$.ajax({
...
function notvia(openId,partTimeJobId){
	if(confirm("确定是否不通过")){
	console.log("用户ID:"+openId+"不通过兼职id:"+partTimeJobId+"（接上数据库后删除）");  
	$.ajax({
		type: "post",  //数据提交方式（post/get）
		url: commentDataUrl,     //这里是请求的后台地址，自己定义
		data: {
