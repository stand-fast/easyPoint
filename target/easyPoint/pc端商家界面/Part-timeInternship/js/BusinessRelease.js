...
if(partTimeJobId=="没有这个参数"){
	...
	{"jobName":"移动公司实习生","jobContent":"负责在上菜区将菜品及时摆出，保证出餐窗口的整洁干净，及时跟进顾客的饮食情况，在结账窗口对顾客的菜品消费进行数目结账","jobSettle":"日结","jobSalary":"130/天","jobPlace":"广东省广州市天河区龙洞迎福路527号广东金融学院北苑饭堂二楼","jobDate":"2019-07-31到2019-08-07","jobTime":"10:00-15:00","endTime":"2019-08-06","recruitNum":"21","welfare":"爱神的箭卡时间看","sex":"不限","requirement":"asdhasjkhdkasjhd就打算开发和接口","legalPerson":"肖奈","phone":"13501212354","companyIntroduce":"发货就肯定会接口","OrderStatus":"1"},           //OrderStatus为审核界面状态，1为正在审核，2为审核未通过
	],"dataType":"3"           //dataType值为1时候为 我的保存 页面的值,dataType值为2时候为 我的审核 页面的值,dataType值为3时候为 我的审核 页面的值,
	}
	...
	if(dataType==2){       	   //dataType值为1时候为 我的保存 页面的值,dataType值为2时候为 我的审核 页面的值,dataType值为3时候为 我的审核 页面的值
		$("#stateJudgment").empty(); 
		var data_html = "<button id='Shelves' class='Part-timeButton save'>删除</button> <button id='uploadReview' class='Part-timeButton uploadReview'>重新审核</button>";
	...
	$.ajax({
			...
			success: function(json){
				...
				if(dataType==3){       	   //dataType值为1时候为 我的保存 页面的值,dataType值为2时候为 我的审核 页面的值,dataType值为3时候为 我的审核 页面的值
					$("#stateJudgment").empty(); 
					var data_html = "<button id='Shelves' class='Part-timeButton save'>下架</button> <button id='uploadReview' class='Part-timeButton uploadReview'>重新审核</button>";
					$("#stateJudgment").append(data_html);
				}
...
function check(Hiring){
	...
	$('#ReAudit').click(function(){
		...
		if(check(Hiring)==true){
			if(confirm("重新审核会清除该兼职所有通过用户的数据"+"\r岗位名称:"+JobTitle+"\r工作内容:"+WorkContent+"\r工作类型:"+JobType+"\r工资:"+Wages+"\r工作地点:"+ WorkPlace+"\r兼职日期:"+departureDayBegin+" 至 "+departureDayEnd+"\r兼职时间:"+departureTimeBegin+" 至 "+departureTimeEnd+"\r截至日期:"+deadline+"\r招聘人数:"+Hiring+"人\r福利待遇:"+WelfareTreatment+"\r性别要求:"+GenderRequirement+"\r任职要求:"+JobRequirements+"\r联系人:"+ContactsPeople+"\r联系电话:"+ContactsPhone+"\r公司介绍:"+CompanyIntroduction)){
				var jobDate=departureDayBegin+" 到 "+departureDayEnd;
				var jobTime=departureTimeBegin+" - "+departureTimeEnd;
				$.ajax({
					type: "post",  //数据提交方式（post/get）
